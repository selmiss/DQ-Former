...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_config_path", type=str, default="configs/stage1/train_config.yaml"
    )
    parser.add_argument(
    ...
    print("-" * 60)
    print(
        f"batch_size: {data_config.batch_size}\tnum_devices: {torch.cuda.device_count() if torch.cuda.is_available() else 0}\taccumulate_grad_batches: {train_config.accumulate_grad_batches}"
    )
    print(
        f"Total batch size: {data_config.batch_size * torch.cuda.device_count() if torch.cuda.is_available() else 0 * train_config.accumulate_grad_batches}"
    )
    if args.test_mode:
