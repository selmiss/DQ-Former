#!/bin/bash
# Retrosynthesis Finetuning Training Script
...
# Usage: ./retrosynthesis.sh     (default - auto-detects based on num_train_epochs)
#        ./retrosynthesis.sh 3   (training with eval)
#        ./retrosynthesis.sh 2   (training without eval)
DEEPSPEED_STAGE=${1:-2}


echo "=========================================="
echo "Retrosynthesis DQ-Former Training"
echo "=========================================="
...
deepspeed --master_port ${MASTER_PORT} --include localhost:${GPUs} \
    ...
    --model_config_path ${BASE_DIR}/configs/qa/retrosynthesis/model_config.yaml \
    --training_config_path ${BASE_DIR}/configs/qa/retrosynthesis/training_config.yaml \
    --data_config_path ${BASE_DIR}/configs/qa/retrosynthesis/data_config_preprocessed.yaml \
