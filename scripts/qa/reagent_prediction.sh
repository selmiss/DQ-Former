#!/bin/bash
# Reagent Prediction Finetuning Training Script
...
# Configuration
export GPUs="0"  # GPU IDs to use for reagent prediction training
...
# Usage: ./reagent_prediction.sh     (default - auto-detects based on num_train_epochs)
#        ./reagent_prediction.sh 3   (training with eval)
#        ./reagent_prediction.sh 2   (training without eval)
DEEPSPEED_STAGE=${1:-2}
...
deepspeed --master_port ${MASTER_PORT} --include localhost:${GPUs} \
    ...
    --model_config_path ${BASE_DIR}/configs/qa/reagent_prediction/model_config.yaml \
    --training_config_path ${BASE_DIR}/configs/qa/reagent_prediction/training_config.yaml \
    --data_config_path ${BASE_DIR}/configs/qa/reagent_prediction/data_config_preprocessed.yaml \
