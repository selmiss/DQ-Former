...
def main(model_config, train_config, data_config, resume_from=None, test_mode=False):
    ...
    if use_llm_baseline:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            train_config.llm_model_path, padding_side="left"
        )
    else:
    ...
    if not train_config.use_dq_encoder:
        cus_model.model = cus_model.model.from_pretrained(
            train_config.ckpt_path, torch_dtype=torch_dtype
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_config_path", type=str, default="configs/moleculeqa/train_config.yaml"
    )
    parser.add_argument(
        "--data_config_path", type=str, default="configs/moleculeqa/data_config.yaml"
    )
    parser.add_argument(
        "--resume_from",
        type=str,
    ...
    print(
        f"Total batch size: {data_config.batch_size * max(1, detected_num_devices) * train_config.accumulate_grad_batches}"
    )
    print("-" * 60)
